...
//	  .setFooter(app.embed.footer, kullanıcı.user.displayAvatarURL({dynamic: true}))
    ...
    let dataTop = data.filter(x => message.guild.members.cache.has(x)).sort((a, b) => Number((teyitData[b].erkek || 0) + (teyitData[b].kız || 0)) - Number((teyitData[a].erkek) + (teyitData[a].kız))).map((value, index) => `\`${index+1}.\` ${message.guild.members.cache.get(value)} **\`${teyitData[value].erkek || 0}\` erkek, \`${teyitData[value].kız || 0}\` kadın [${(teyitData[value].erkek || 0) + (teyitData[value].kız || 0)}]**`).splice(0, 15).join("\n");
